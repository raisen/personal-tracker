...
// Register all tools on a fresh McpServer instance
function createTrackerServer(): McpServer {
...
// --- HTTP Server with Streamable HTTP Transport (stateless, per-request) ---
...
const httpServer = createServer(async (req, res) => {
  ...
  if (url.pathname === "/mcp") {
    // Stateless: GET and DELETE are not supported
    if (req.method === "GET") {
      res.writeHead(405, { "Content-Type": "application/json" });
      res.end(JSON.stringify({
        jsonrpc: "2.0",
        error: { code: -32000, message: "Method not allowed. Use POST for stateless server." },
        id: null,
      }));
      return;
    }
